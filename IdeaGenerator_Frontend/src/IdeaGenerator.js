...
const steps = ['subdomain', 'technologies', 'business_model', 'target_audience', 'market_segment'];
...
const IdeaGenerator = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setIndustries((await industriesRes.json()).industries || []);
        setFocusOptions((await focusRes.json()).focus_options || []);
      } catch (err) {
  ...
  const handleGenerateIdeas = async (count) => {
    ...
    try {
      ...
      await fetch('http://localhost:8000/api/save-full-idea/', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...formData,
  ...
  const renderFormInputs = useCallback(
      ...
      <div className="form-center-panel">
        ...
        <div className="select-container">
          <select
            ...
            aria-label="Select primary focus"
          >
          ...
          <span className="KILLselect-arrow">▼</span>
        </div>
          ...
          <div className="form-group">
            ...
            <div className="select-container">
              <select
                ...
                aria-label="Select primary industry"
              >
                <option value="" disabled>
                  Select industry
                </option>
          ...
          <div className="form-group">
            ...
            <div className="select-container">
              <select
                ...
                aria-label="Select sub-industry"
              >
                <option value="" disabled>
                  Select sub-industry
                </option>
          ...
          <div className="form-group">
            ...
            <div className="select-container">
              <select
                ...
                aria-label="Select business model"
              >
                <option value="" disabled>
        ...
        <div className="generate-buttons">
          <button
            ...
            aria-label="Generate 5 startup ideas"
          >
          ...
          <button
            ...
            disabled={!isFormValid() || isGeneratingIdeas}
            aria-label="Generate 10 startup ideas"
          >
    ...
    [formData, focusOptions, industries, options, isGeneratingIdeas, isFormValid, handleGenerateIdeas]
  );
    ...
    () => (
      <div className="sidebar-panel">
        <div className="sidebar-logo">
          YAIIA <span className="logo-dot">•</span>
        </div>
  ...
  // Render ideas panel
  const renderIdeasPanel = useCallback(
    () => (
      <div className="ideas-panel">
          ...
          <div className="idea-details">
            {generatedIdeas.map((idea, index) => (
              <div
                ...
                role="button"
                tabIndex={0}
                onKeyPress={(e) => e.key === 'Enter' && handleGenerateSolution(idea.description)}
                aria-label={`View solution for idea ${index + 1}`}
              >
                <h3>
                  {index + 1}. {idea.title}
                </h3>
        ...
        {/* Full-screen solution view modal */}
    {selectedIdea && (
      <div className="solution-display">
        <div className="solution-header">
          <h2>Startup Solution</h2>
          <button className="close-btn" onClick={() => setSelectedIdea(null)} aria-label="Close solution view">×</button>
        </div>
        ...
        <div className="solution-sections">
          {selectedIdea.solution.split(/\n\s*\d+\.\s+/).map((sectionText, index) => {
            const sectionTitle = selectedIdea.solution.match(/\d+\.\s+([^\n]+)/g)?.[index]?.replace(/^\d+\.\s+/, '') || `Section ${index + 1}`;
            return (
              <div key={index} className="solution-section">
        ...
        <div className="solution-actions">
          <button className="back-to-ideas" onClick={() => setSelectedIdea(null)}>
            ← Back to Ideas
          </button>
          <button className="save-solution">
            💾 Save Solution
          </button>
        </div>
      </div>
    ...
    [generatedIdeas, selectedIdea, handleGenerateSolution]
  );
